...
#define AUX_FLAG 4
#define FR_FLAG_RISE 5
#define FR_FLAG_FALL 6
#define FL_FLAG_RISE 7
#define FL_FLAG_FALL 8
...
// Variables used to record the sensor values
uint16_t lineFR;
...
void setup()
{
  ...
  PCintPort::attachInterrupt(IR_FR, irFR, CHANGE);
  PCintPort::attachInterrupt(IR_FL, irFL, CHANGE);
}
...
void loop()
{
  ...
  if (lineFR < 300)
  {
    CRCArduinoFastServos::writeMicroseconds(SERVO_THROTTLE, 1000);
    CRCArduinoFastServos::writeMicroseconds(SERVO_STEERING, 1000);
    delay(150);
  }
  else
  {
    ...
    {
      BUSCA = 0;
      CRCArduinoFastServos::writeMicroseconds(SERVO_THROTTLE, 1500);
      CRCArduinoFastServos::writeMicroseconds(SERVO_STEERING, 1500);
  ...
  delay(2);
  Serial.print(lineFR); Serial.print("  "); Serial.print(unThrottleIn); Serial.print(" "); Serial.println(unSteeringIn);
  bUpdateFlags = 0;
}
